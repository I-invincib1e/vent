...
## Where Vent sits: self-hosted orchestration, bring-your-own AI providers

Voice-agent architectures sit on a spectrum:

```
Fully local                    Vent                       Fully managed
...
Vent self-hosts the *orchestration layer* — the code, the database, the call logic, the compliance rules,
...
What you actually own at Vent's position: which provider plugs into each slot (swap Deepgram, swap the
...
you don't own, and Vent never claims to: the phone network itself, or the AI model weights the LLM/STT/TTS
calls run against.
